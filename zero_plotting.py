#!/usr/bin/env python3
"""
Zero Motorcycle Log Plotting Module
...
from zero_log_parser import BinaryTools
...
class ZeroLogPlotter:
    ...
    def __init__(self, input_file: str):
        ...
        if not PLOTLY_AVAILABLE:
            raise ImportError("plotly and pandas are required for plotting. Install with: pip install plotly pandas")
    ...
    def _load_from_binary(self):
        ...
        from zero_log_parser import main as parser_main
        import sys
        ...
        try:
            sys.argv = ['zero_log_parser.py', self.input_file, '-f', 'csv', '-o', temp_csv.name]
            parser_main()
        finally:
...
def main():
    """Command-line interface for Zero log plotting."""
    if not PLOTLY_AVAILABLE:
        print("Error: plotly is required for plotting. Install with: pip install plotly pandas")
        return 1
    
    parser = argparse.ArgumentParser(description='Generate interactive plots from Zero Motorcycle logs')
    parser.add_argument('input_file', help='Input file (.bin or .csv)')
